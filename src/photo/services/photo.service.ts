...
class PhotoService {
    ...
    async getAll() {
        ...
        const { data } = await axiosClassic.get<IPhotoRdo[]>(API_URL.files('/photos'))
        return data
    }
    ...
    async getById(photoId: string) {
        ...
        const { data } = await axiosClassic.get<IPhotoDetails>(API_URL.files(`/photos/${photoId}`))
        return data
    }
    ...
    async upload(data: ICreatePhotoForm) {
        ...
        const { data: createdPhoto } = await axiosWithAuth.post<IPhotoRdo>(API_URL.files('/photos'), formData, {
            headers: {
    ...
    async addComment(photoId: string, data: ICreateCommentForm) {
        ...
        const url = API_URL.files(`/photos/${photoId}/comments`)
